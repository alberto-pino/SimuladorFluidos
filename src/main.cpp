...
static void DrawVelocity(void)
{
	...
	double compx, compy, x,y;
	double h = (double)((double)1 / (double)(N + 2));
	float color;
	...
	for (int i = 1; i < N+2; i++)
	{
		...
		{
			y = (double)(j - 0.5f)*h;
			compx = solver.u[XY_TO_ARRAY(i, j)];
			compy = solver.v[XY_TO_ARRAY(i, j)];
			color = fmin(compx+compy, 1);

			glColor3f(compx, compy, 1.0f);

			glVertex2f(x, y);
			glVertex2f(x+(compx/50), y+(compy/50));
...
static void DrawDensity(void)
{
	...
	for (int i = 0; i <= (N + 1); i++)
	{
		...
		for (int j = 0; j <= (N+1); j++)
		{
			...
			color = sqrtf(sqrtf(solver.dens[XY_TO_ARRAY(i, j)]));
